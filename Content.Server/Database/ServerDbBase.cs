...
namespace Content.Server.Database
{
    public abstract partial class ServerDbBase
    {
            ...
            CancellationToken cancel = default)
        {
            ...
            {
                profiles[profile.Slot] = ConvertToCharacter(profile);
            }
        ...
        public async Task SaveCharacterSlotAsync(NetUserId userId, ICharacterProfile? profile, int slot)
        {
            ...
            var newProfile = ConvertToDatabaseProfile(profile, slot, oldProfile);
            if (oldProfile == null)
            {
                var prefs = await db.DbContext
                    .Preference
        ...
        public async Task<PlayerPreferences> InitPrefsAsync(NetUserId userId, ICharacterProfile defaultProfile)
        {
            ...
            var profile = ConvertHumanoidToDatabaseProfile((HumanoidCharacterProfile) defaultProfile, 0);
        ...
        private BaseProfile ConvertToDatabaseProfile(ICharacterProfile profile, int slot, BaseProfile? oldProfile)
        {
            return profile switch
            {
                HumanoidCharacterProfile humanoid => ConvertHumanoidToDatabaseProfile(humanoid, slot, oldProfile),
                BorgCharacterProfile borg => ConvertBorgToDatabaseProfile(borg, slot, oldProfile),
        ...
        private static ICharacterProfile ConvertToCharacter(BaseProfile profile)
        {
            return profile switch
            {
                HumanoidProfile humanoid => ConvertToHumanoidCharacter(humanoid),
                BorgProfile borg => ConvertToBorgCharacter(borg),
                _ => throw new NotImplementedException($"Profile type {profile.GetType()} not implemented")
            };
        }
